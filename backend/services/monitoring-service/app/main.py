...
# Include routers
app.include_router(routes.router, prefix="/api/v1/monitoring", tags=["monitoring"])
...
def root():
    """Root endpoint"""
    return {"message": "Monitoring Service API", "version": "1.0.0"}

if __name__ == "__main__":
    uvicorn.run(
        "main:app",
        host="0.0.0.0",
        port=settings.MONITORING_SERVICE_SERVICE_PORT,
        reload=True
    )
